...
function calcNextNumber(sequence: number[]): number {
	...
	for (let i = sequenceDifferentials.length - 2; i >= 0; i--) {
		...
		const lastCurrNum = currSeq.at(0) as number
		const lastDiffNum = diffSeq.at(0) as number

		currSeq.unshift(lastCurrNum - lastDiffNum)
	}

	return sequenceDifferentials[0].at(0) as number
}
